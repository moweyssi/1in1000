...
company_emissions  = read.csv("data/company_emissions.csv")
assets             = read.csv("data/assets.csv")
...
#Split the data frame into two depending if unit is power or energy
...
#Same pivot_longer operation as the activities data frame
...
#Join all three data frames together, perform column operations and rename
assets_new = company_activities_MWh_tcoal %>% 
  ...
  left_join(company_emissions_long,
            by=c("company_id","company_name","ald_sector","ald_business_unit","Year")) %>% 
  #Calculate emissions in correct units
  ...
  rename(plan_tech_prod = Activities.Equity.Ownership_MW,
         year = Year) %>% 
  select(names(assets)) %>% 
  #Add unique record ID for troubleshooting
  mutate(record_ID = paste(year,company_id,
                           ald_sector,ald_business_unit,
                           ald_production_unit,emissions_factor_unit,
                           scenario_geography,sep="__"))

#Filter correct year range of the reference data for comparison
assets_old_correct_range = assets %>% 
  filter(year<=2026 & year>=2024) %>% 
  mutate(record_ID = paste(year,company_id,ald_sector,ald_business_unit,
                           ald_production_unit,emissions_factor_unit,scenario_geography,sep="__"))
...
#We can compare the old and new dataframe directly
